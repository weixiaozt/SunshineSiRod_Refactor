                ...
                try
                {
                    if (_strSiliconSerialNum != "" && _bGotLeftSiliconData == false)
                    {
                        ...
                        if (true == GlobalDataCache.Instance().GetCheckData(_strSiliconSerialNum, ref _curStickData))
                        {
                            ...
                            labelLeftLeftDiag.Text = "2_Projectio1：" + strLLDiagLength;
                            ...
                            {
                                strTopAngle += ((float)item).ToString("0.00");
                                if ((float)item < 89.90)
                                {
                                   // bValid = true;
                                }
                            ...
                            {
                                strLeftAngle += ((float)item).ToString("0.00");
                                if ((float)item < 89.90)
                                {
                                   // bValid = true;
                                }
                            ...
                            {
                                strRightAngle += ((float)item).ToString("0.00");
                                if ((float)item < 89.90)
                                {
                                   // bValid = true;
                                }
                            ...
                            {
                                strDownAngle += ((float)item).ToString("0.00");
                                if ((float)item < 89.90)
                                {
                                   // bValid = true;
                                }
                            ...
                            try
                            {
                               ...
                               // string url = "http://10.45.248.216:5000/api/OrBitWebAPILogin";
                                string url = "http://10.45.248.216:5000/api/OrBitWebAPI";
                                var formData = new MultipartFormDataContent();
                                formData.Add(new StringContent("BrickVisionInspectionDataPilot"), "APIName");
                                formData.Add(new StringContent(_token), "Token");
                                formData.Add(new StringContent("T1N12H25120014D1220SR"), "sn");
                                DateTime dateTime1 = DateTime.Now;
                                string strDateTime1 = dateTime1.ToString("yyyy-MM-dd HH:mm:ss");
                                string userDataJson = JsonSerializer.Serialize(new
                                {
                                    SerialNum = _strSiliconSerialNum,
                                    LTLength = strLTLength,
                                    RTLength = strRTLength,
                                    LDLength = strLDLength,
                                    RDLength = strRDLength,
                                    TDLength = strTDLength,
                                    LRLength = strLRLength,
                                    TopDiag = strTDiagLength,
                                    LeftDiag = strLDiagLength,
                                    RightDiag = strRDiagLength,
                                    DownDiag = strDDiagLength,
                                    TopLeftDiag = strTLDiagLength,
                                    TopRightDiag = strTRDiagLength,
                                    LeftLeftDiag = strLLDiagLength,
                                    LeftRightDiag = strLRDiagLength,
                                    RightLeftDiag = strRLDiagLength,
                                    RightRightDiag = strRRDiagLength,
                                    DownLeftDiag = strDLDiagLength,
                                    DownRightDiag = strDRDiagLength,
                                    TopAngle = strTopAngle,
                                    LeftAngle = strLeftAngle,
                                    RightAngle = strRightAngle,
                                    DownAngle = strDownAngle,
                                    Result = "1",
                                    length = strLengthInfo,
                                    SVer = S_Ver,
                                    EVer = E_Ver,
                                    TopLength = "1",
                                    RightLength = "1",
