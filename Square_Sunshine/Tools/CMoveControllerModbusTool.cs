...
{
    internal class CMoveControllerModbusTool
    {
        private static CMoveControllerModbusTool _instance;
        private EasyModbus.ModbusClient _modbusClient;
        public string receiveData = null;

        public static CMoveControllerModbusTool Instance()
        {
            if (null == _instance)
            {
                _instance = new CMoveControllerModbusTool();
            }    
        ...
        private CMoveControllerModbusTool()
        {
        ...
        public bool WriteTwoRegister(int nStartIndex, float fRegisterInfo)
        {
            ...
            catch (Exception e)
            {
                ...
                FormMain.formMainF.showMessageDelegate.Invoke("modbus disconnect Reconnect!", (int)FormMain.emMSGTYPE.EM_MODBUSDISCONNECT);
            }
        ...
        public bool ConnectServer()
        {
            try
            {
                _modbusClient.IPAddress = SettingParameter.Instance().StrIPMoveControl;
                _modbusClient.Port = SettingParameter.Instance().NMoveControlPort;
                _modbusClient.Connect();
                LogHelper.Info("Silicon", "Connect MoveController Server Sucesss" + _modbusClient.IPAddress + " " + _modbusClient.Port + " Connected " + _modbusClient.Connected.ToString());
        ...
        private void UpdateConnectedChanged(object sender)
        {
            ...
            else
            {
                ...
                FormMain.formMainF.showMessageDelegate.Invoke("modbus DisConnected to Modbus Server", (int)FormMain.emMSGTYPE.EM_MODBUSDISCONNECT);
