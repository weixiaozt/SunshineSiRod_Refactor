...
function renderDriftStatus() {
  ...
  panel.className = `drift-status-panel ${status === 'abnormal_corrected' ? 'corrected' : status === 'unmatched_unadjusted' ? 'warning' : 'normal'}`;
  ...
  panel.innerHTML = `<strong>${cn ? '\u673a\u68b0\u6f02\u79fb' : 'Mechanical drift'}: ${labels[status] || status}</strong><span>${cn ? '\u5e45\u5ea6' : 'Amplitude'} ${format(drift.drift_amplitude)} · ${cn ? '\u7f6e\u4fe1\u5ea6' : 'Confidence'} ${format(Number(drift.drift_confidence) * 100)}% · RMSE ${format(drift.drift_fit_rmse_mm)} mm · ${cn ? '\u91cd\u53e0\u7ad9\u70b9' : 'Overlap stations'} ${format(drift.drift_sample_station_count)} (${format(Number(drift.drift_overlap_start_fraction) * 100)}–${format(Number(drift.drift_overlap_end_fraction) * 100)}%) · v${drift.drift_model_version || '—'}</span>${reason}<div class="drift-camera-values">${cameraDetails}</div>`;
}
    ...
    try {
      ...
      if (state.result?.raw_summary) {
        ...
        ['head','tail'].forEach(end => {
          ...
          if (angles.length === 4) corrected[`${end}_endface_verticality_deg`] = (angles.reduce((sum, value) => sum + value, 0) / 4).toFixed(6);
          else corrected[`${end}_endface_verticality_deg`] = '';
        });
