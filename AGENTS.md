...
## ABCD边长正式算法：长棒模板标定长度算法（2026-07-17）

除已经冻结的同事独立离线交付包历史产物外，本项目后续开发、验证和接入Web的
`A/B/C/D`边长统一使用：

```text
算法名称：长棒模板标定长度算法
独立源码：ABCD_length6图完美模板标定法\
正式模型：ABCD_length6图完美模板标定法\models\long_rod_template_calibrated_length_210_105.json
模型版本：4
SHA256：311AC6F3C0E5998E4C43B94D4FAF271FC53B4742C92FDC321801312C57B120CC
```

固定计算口径：四相机先按正式固定Y偏移插值到共同物理Y；A/C取全部有效站点平均；
B/D分别取头部100片和尾部100片均值后再平均。只允许一个B/D设备链相对K：

```text
B_final = B_raw + K/2
D_final = D_raw - K/2
K = -0.00702230141682018 mm
```

K先在CTB六图3+3、CMM长棒10+9（明确排除碎角`14_16.hobj`）、GTT
`10_16/10_56`三组内分别由`正向B↔调头D、正向D↔调头B`交叉自洽关系求值，
再让三根棒等权平均。完整推导由`build_final_abcd_model.py`从HOBJ重算，诊断保存在
`results\long_rod_template_length_algorithm\k_derivation.csv`。不得改回仅CTB的
`+0.000148...`，也不得直接复用旧CMM实验的`-0.010072...`。

运行时永远不得从文件名、父目录或提示词判断是否调头，不执行A/B/C/D、头尾交换；
明确分组只允许用于标定后验证和K构建。每次测量必须保留加K前raw与最终值，K对B/D
等量反向作用并保持每片`B+D`不变，禁止增加A/B/C/D四通道独立终值补偿。
